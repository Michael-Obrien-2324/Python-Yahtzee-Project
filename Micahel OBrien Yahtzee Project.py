...
def rank(): #This functon ranks the players based on amount of points currently
    org()
    print('\nRight now...')
    ...
    if points[n]!=0:
        ...
        for i in range(5):
            if dice[i]==6:
                points[n]+=6
        return points[n]
def tok(x):  #TOK is Three of a Kind
    n=(x*13)+6
    if points[n]!=0:
...
def yaht(x):	#Yahtzee
    n=(x*13)+11
    if points[n]!=0:
        p=print('\nSorry you already used your yahtzee space, try a different spot.')
        return -1
    else:
